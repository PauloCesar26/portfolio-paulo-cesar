    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-slot="sidebar"
          data-mobile="true"
          className="bg-zinc-300 text-black dark:bg-zinc-950 dark:text-white w-(--sidebar-width) p-4 [&>button]:hidden"
          style={
            {
    ...
    <div
      ...
      <div
        ...
        <div
          ...
          className="p-4 bg-zinc-300/55 text-black dark:bg-zinc-950 dark:text-white transition duration-700 ease-in-out group-data-[variant=floating]:border-sidebar-border flex h-full w-full flex-col group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:shadow-sm">
          {children}
        </div>
      </div>
    </div>
...
function SidebarTrigger({
  ...
  useEffect(() => {
    //function que lida com o tamanho da janela do navegador, toda vez que a janela mudar de tamanho a function é chamada
    const handleResize = () => {
      setIsMobile(window.innerWidth <= 768)
    }
    //quando a tela for para o modo mobile a function handleResize será chamada
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        "text-sidebar-foreground/70 ring-sidebar-ring flex shrink-0 items-center rounded-md px-2 text-sm outline-hidden transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
...
function SidebarMenu({
  ...
  return (
    <ul
      ...
      data-sidebar="menu"
      className={cn("flex w-full min-w-0 flex-col gap-2", className="mt-2")}
      {...props} />
  );
}
